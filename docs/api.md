...
| POST | `/api/external/mailboxes/claim` | API Key | JSON | 从普通邮箱或临时邮箱分组领取一个可用资源 |
| POST | `/api/external/mailboxes/complete` | API Key | JSON | 完成领取并移动资源到目标分组 |
| POST | `/api/external/mailboxes/release` | API Key | JSON | 主动释放领取中的资源 |
...
| GET | `/api/temp-emails/<temp_email_id>/shares` | Session | JSON | 获取临时邮箱分享链接 |
| POST | `/api/temp-emails/<temp_email_id>/shares` | Session + CSRF | JSON | 创建临时邮箱分享链接 |
| DELETE | `/api/temp-emails/<temp_email_id>/shares/<share_id>` | Session + CSRF | JSON | 删除临时邮箱分享链接 |
| GET | `/api/shared/<token>` | 无 | JSON | 公开查看分享临时邮箱信息 |
| GET | `/api/shared/<token>/messages` | 无 | JSON | 公开查看分享临时邮箱邮件列表 |
| POST | `/api/shared/<token>/refresh` | 无 | JSON | 公开刷新分享临时邮箱邮件，30 秒节流 |
| GET | `/api/shared/<token>/messages/<message_id>` | 无 | JSON | 公开查看分享临时邮箱邮件详情 |
...
从指定分组领取一个可用邮箱资源。源分组的 `mailbox_type` 决定领取普通账号还是临时邮箱：`account` 分组领取 `accounts`，`temp_email` 分组领取 `temp_emails`。

鉴权：对外 API Key，支持 Header `X-API-Key` 或查询参数 `api_key`。

#### 请求字段

| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `source_group_id` | int | 是 | 源分组 ID，必须存在 |
| `caller_id` | string | 是 | 调用方标识，用于日志和排查 |
| `task_id` | string | 是 | 外部任务标识 |
| `lease_seconds` | int | 否 | 租约秒数，默认 `600`；小于 `1` 按 `1`，大于 `3600` 按 `3600` |
...
成功时返回 `success: true` 和 `mailbox`。无可领取资源时 `mailbox` 为 `null`，仍返回 HTTP `200`。
...
| 场景 | HTTP | 示例 |
| --- | --- | --- |
| 缺少或错误 API Key | `401`/`403` | 同 `/api/external/verification-code` |
| 缺少或非法 `source_group_id` | `400` | `{"success": false, "error": "缺少 source_group_id"}` |
| 缺少 `caller_id` 或 `task_id` | `400` | `{"success": false, "error": "缺少 caller_id"}` |
| `lease_seconds` 不是整数 | `400` | `{"success": false, "error": "lease_seconds 必须为整数"}` |
| 源分组不存在 | `404` | `{"success": false, "error": "源分组不存在"}` |
| 生成领取 token 冲突 | `409` | `{"success": false, "error": "生成领取 token 失败"}` |

### POST `/api/external/mailboxes/complete`

完成一次领取，并把资源移动到目标分组。目标分组的 `mailbox_type` 必须与 `resource_type` 一致：普通账号只能完成到普通邮箱分组，临时邮箱只能完成到临时邮箱分组。
...
| 缺少或非法 `target_group_id` / `resource_id` | `400` | `{"success": false, "error": "target_group_id 必须为正整数"}` |
| `resource_type` 无效 | `400` | `{"success": false, "error": "resource_type 无效"}` |
| 目标分组类型不匹配 | `400` | `{"success": false, "error": "目标分组类型不匹配"}` |
| 目标分组不存在 | `404` | `{"success": false, "error": "目标分组不存在"}` |
| 邮箱资源不存在 | `404` | `{"success": false, "error": "邮箱资源不存在"}` |
| token 不存在、已完成、已释放，或资源已被其他有效领取占用 | `409` | `{"success": false, "error": "领取 token 状态冲突"}` |

### POST `/api/external/mailboxes/release`

主动释放领取中的资源，资源留在原分组等待后续重新领取。
...
```bash
curl -X POST "http://localhost:5000/api/external/mailboxes/release" \
  -H "X-API-Key: your-api-key" \
  -H "Content-Type: application/json" \
  -d '{"resource_type":"account","resource_id":123,"claim_token":"mclm_xxx","caller_id":"worker-01","task_id":"batch-001"}'
```
...
- `claim` 只领取 `status='active'` 且当前没有有效 `claiming` 记录的资源，并在事务内写入 `claim_token` 和 `lease_expires_at`。
- 租约超时采用惰性回收：服务端不会后台定时扫描，而是在下一次 `claim`、`complete` 或 `release` 时把到期的 `claiming` 标记为 `expired`。
- 迟到 `complete` 在资源未被重新领取时允许：只要旧 `claim_token` 仍匹配同一资源，且该资源没有其他有效 `claiming` 记录，即使旧记录已变成 `expired` 也可以完成并移动资源。
- 迟到 `release` 在 claim 已 `expired` 时返回 `409`；`release` 只接受当前仍处于 `claiming` 的记录。
- 如果资源在旧 token 到期后已经被新的 claim 占用，旧 token 的 `complete` 和 `release` 都返回 `409`。
...
| POST | `/api/groups` | JSON: `name`、`description?`、`color?`、`proxy_url?`、`sort_position?`、`mailbox_type?` | 创建分组 |
| PUT | `/api/groups/<group_id>` | JSON: `name`、`description?`、`color?`、`proxy_url?`、`sort_position?`、`mailbox_type?` | 更新分组 |
| DELETE | `/api/groups/<group_id>` | 路径参数 `group_id` | 删除分组，默认分组不能删除 |
| PUT | `/api/groups/reorder` | JSON: `group_ids: number[]` | 重新排序普通分组 |

创建或更新分组请求示例：

`mailbox_type` 可选值为 `account` 或 `temp_email`，默认 `account`。一个分组只能包含一种资源；已有资源的分组不能直接改成另一种类型。
...
| 方法 | 路径 | 参数 | 说明 |
| --- | --- | --- | --- |
| GET | `/api/temp-emails` | Query: `group_id?` | 获取临时邮箱列表，列表项包含 `tags` 字段；传 `group_id` 时仅返回指定临时邮箱分组 |
| POST | `/api/temp-emails/import` | JSON: `account_string`、`provider`、`group_id?` | 批量导入临时邮箱 |
...
登录后可以为任意 GPTMail、DuckMail、Cloudflare 临时邮箱创建多个分享链接。分享链接形如 `/shared/<token>`，访问者无需登录即可只读查看邮件列表和详情，并可手动刷新。

#### 登录态分享管理

| 方法 | 路径 | 请求参数 | 说明 |
| --- | --- | --- | --- |
| GET | `/api/temp-emails/<temp_email_id>/shares` | 路径参数 `temp_email_id` | 获取该临时邮箱的分享链接列表 |
| POST | `/api/temp-emails/<temp_email_id>/shares` | JSON: `expires_in?` | 创建分享链接 |
| DELETE | `/api/temp-emails/<temp_email_id>/shares/<share_id>` | 路径参数 | 删除分享链接，删除后公开访问立即失效 |

`expires_in` 单位为毫秒，只接受以下值：
...
| 方法 | 路径 | 鉴权 | 说明 |
| --- | --- | --- | --- |
| GET | `/api/shared/<token>` | 无 | 获取分享邮箱公开信息 |
| GET | `/api/shared/<token>/messages` | 无 | 获取缓存邮件列表 |
| POST | `/api/shared/<token>/refresh` | 无 | 刷新邮件，按 token 30 秒节流 |
| GET | `/api/shared/<token>/messages/<message_id>` | 无 | 获取单封邮件详情 |
  ...
  "emails": [
    {
      ...
      "body_preview": "Your code is 123456",
      "date": "2026-06-01 12:01:00",
      "timestamp": 1780296060,
      "has_html": 1
    }
  ],
  "count": 1
...
如果同一个 token 在 30 秒内重复刷新，后端不会请求上游服务，会直接返回缓存邮件并带 `throttled=true`。刷新失败时错误固定为 `刷新邮件失败`，不会暴露上游接口响应。

公开接口不会返回 DuckMail 密码、DuckMail Token、Cloudflare JWT、Refresh Token 或其他 provider 凭据。过期分享返回 HTTP `410`，不存在或已删除的分享返回 HTTP `404`。
