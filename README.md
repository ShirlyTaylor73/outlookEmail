...
临时邮箱支持生成只读分享链接，适用于把验证码收信页面临时交给他人查看。

1. 打开「临时邮箱」分组。
2. 点击某个临时邮箱右侧三点菜单，选择「分享」。
3. 选择有效期：1 小时、24 小时、3 天、1 个月或永久。
4. 点击「创建分享链接」，复制生成的 `/shared/<token>` 链接。
5. 访问者无需登录即可查看该临时邮箱的邮件列表和详情，也可以手动刷新；公开刷新按分享链接做 30 秒节流。

公开访问者只能查看和刷新邮件，不能删除邮件、修改邮箱、查看 DuckMail Token、Cloudflare JWT、密码或其他临时邮箱凭据。
